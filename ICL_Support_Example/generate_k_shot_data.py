...
def main():
    ...
    main_for_gao(args, [task for task in args.task
                        if task in ["SST-2", "sst-5", "mr", "cr", "trec", "subj", 'atis', '20News', 'NewsCategory', 'liu', 'facebook', 'snips']])
...
def main_for_gao(args, tasks):
    ...
    for seed in args.seed:
        ...
        for task, dataset in datasets.items():
            ...
            # Write test splits
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                # this if block is for generating full test dataset, and the following if block is for generating the train and dev dataset.
                # GLUE style
                ...
                for split, lines in dataset.items():
                    # print('in first if block, split:{}'.format(split))
