...
N_LABELS_DICT = {"SST-2": 2, "sst-5": 5, "mr": 2, "cr": 2, "mpqa": 2,
                 ...
                 '20News': 21, 'NewsCategory': 42, 'atis': 14, 'facebook': 12, 'liu': 58, 'snips': 7}
...
def load_data_by_fp(task, data_fp=None, split='train'):
    if data_fp == None:
        ...
        elif task in ['20News', 'NewsCategory', 'atis', 'liu', 'facebook', 'snips']:
            data_fp = f'data/original/{task}/train.csv'
        else:
        ...
        if data_fp.endswith('.tsv'):
            ...
            with open(data_fp) as f:
                ...
                # elif task == 'atis':
                    ...
                    'facebook': {
                        'get_info_road_condition':  0,
                        'get_estimated_departure':  1,
                        'get_estimated_duration':   2,
                        'get_event':                3,
                        'get_estimated_arrival':    4,
                        'get_directions':           5,
                        'get_distance':             6,
                        'get_info_traffic':         7,
                        'update_directions':        8,
                    ...
                    'liu': {
                        ...
                        'news_query':         11          ,
                        'general_dontcare':     12            ,
                        'general_commandstop':    13              ,
                        'calendar_query':           14        ,
                        'alarm_query':                15  ,
                        'recommendation_movies':        16            ,
                        'play_audiobook':                 17  ,
                        'weather_query':                    18,
                        'general_praise':   19                ,
                        'social_post':        20          ,
                        ...
                        'takeaway_query':           49        ,
                        'play_game':                  50  ,
                        'email_querycontact':           51        ,
                        'music_query':         52         ,
                        'audio_volume_other':    53               ,
                        'audio_volume_down':       54             ,
                        'music_settings':            55       ,
                        'general_greet':               56     ,
                        'music_dislikeness':             57       ,
                    },
                    'snips': {
                        'AddToPlaylist':            0,
                        'GetWeather':               1,
                    ...
                    for _, text, label in csv.reader(f):
                        if label == 'label':
                            continue
                        label = label_mapper[task][label]
...
def main(logger, args):
    ...
    long_datasets = ["cr", "subj", "agnews",
                     ...
                     "dbpedia", "yahoo"]
    max_length = 256 if train_task in long_datasets else 64
    max_length = {
        'NewsCategory': 20,
            '20News': 100,
            'atis': 20,
