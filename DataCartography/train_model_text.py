...
from nets.text_transformer import *
...
# Basic arguments
parser.add_argument('--dataset', type=str, default='20News', help='dataset')
...
for repeat in range(REPEATS):
    ...
    num_classes, class_names, dst_train, dst_test = dataset_constr[DATASET](dataset_path, tokenizer, MAX_LEN)
    pretrained = True

    model = TextTransformer('bert-base-uncased', num_classes)
    model.cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        model.eval()
        eval_loader = torch.utils.data.DataLoader(dst_train, batch_size=64, shuffle=False, num_workers=1, pin_memory=False)
        for batch_ids, data in enumerate(eval_loader):
            ...
            for idx, output in enumerate(cpu_outputs):
                guid = trainset_permutation_inds[batch_ids * 64 + idx]
