...
from DataLoader.data_loader_within_domain_fsl import CategoriesSampler, \
    FewshotTextDataset, extract_info, extract_text_info, train_test_split_classes, process_labels, extract_train_valid_test_info, extract_strategy_samples_info, TestFewshotTextDataset, TestCategoriesSampler
...
class WithinDomainFewShotLearningExperiment:
    ...
    def configure(self):
        ...
        val_dataset = FewshotTextDataset(md_path=valid_path, label_col=label_col, text_col=file_col, allowed_labels=classes, max_len=MAX_LEN, tokenizer=tokenizer)
        ...
        print('Test dataset')
        test_dataset = FewshotTextDataset(md_path=test_path, label_col=label_col, text_col=file_col, allowed_labels=classes, max_len=MAX_LEN, tokenizer=tokenizer)
        ...
        if strategy == 'basic':
            if self.args.model in ("tfs", "finetuning"):
                ...
                # Batch data set
                train_dataset = FewshotTextDataset(md_path=train_path, label_col=label_col, text_col=file_col, allowed_labels=classes, max_len=MAX_LEN, tokenizer=tokenizer)
            ...
            if self.args.model in ("tfs", "finetuning"):
                ...
                # Batch data set
                train_dataset = TestFewshotTextDataset(train_path, valid_path, label_col, file_col, MAX_LEN, tokenizer)
                self.train_loader = iter(self.cycle(DataLoader(dataset=train_dataset, batch_size=self.conf['train_batch_size'], shuffle=True, num_workers=2)))
    ...
    def run_test(self):
        ...
        for strategy in args.strategies:
            ...
            for run in range(num_runs):
                ...
                dataset = TestFewshotTextDataset(train_path, test_path, label_col, file_col, MAX_LEN, tokenizer)
