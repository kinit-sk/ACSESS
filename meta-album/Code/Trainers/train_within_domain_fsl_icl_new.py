...
from DataLoader.data_loader_within_domain_fsl import CategoriesSampler, \
    FewshotTextDataset, extract_info, extract_text_info, train_test_split_classes, process_labels, extract_train_valid_test_info, extract_strategy_samples_info, TestFewshotTextDataset, TestCategoriesSampler, TestCategoriesICLSampler, TestFewshotICLDataset
...
from Models.protonet_text import PrototypicalNetwork
from Models.matchingnet import MatchingNetwork
from Models.maml_text import MAML
...
from utils.configs import TFS_CONF, FT_CONF, MAML_CONF, MAML_CONF_TEXT, PROTO_CONF, \
    MATCHING_CONF
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--model", choices=["tfs", "finetuning", "maml", "protonet", "matchingnet", "mistral", "zephyr"], required=True, help="which model to use")
    ...
    parser.add_argument("--backbone", type=str, default="bert-base-uncased", help="backbone to use. Default: resnet18.")
    ...
    parser.add_argument("--test_opt", choices=["adam", "sgd"], default="adam",
        ...
        "Default: adam.")
    parser.add_argument("--img_size", type=int, default=128, help="size of the images. Default: 128.")
    parser.add_argument("--hidden_size", type=int, default=64, help="size of the images. Default: 64.")
    parser.add_argument("--max_len", type=int, default=50, help="size of the texts. Default: 50.")
...
class WithinDomainFewShotLearningExperiment:
    ...
    def configure(self):
        ...
        mod_to_conf = {
            ...
            "maml": (MAML, deepcopy(MAML_CONF_TEXT)),
        ...
        self.conf["baselearner_fn"] = TextTransformer

        if self.args.train_iters is None:
            self.args.train_iters = 60000 * self.conf["meta_batch_size"]

        (file_col, label_col, train_data) = extract_text_info(self.data_dir, self.args.dataset)
        meta_data = pd.read_csv(train_data)
        ...
        for run in range(self.args.runs):
            ...
            if model.trainable:
                ...
                for i, task in enumerate(self.train_loader):
                    ...
                    # if ((i+1) == self.args.train_iters and self.args.model not in ('tfs', 'finetuning')) or ((i+1) == int(self.args.train_iters * 1.5)):
    ...
    def run_test(self):
        ...
        model = self.model_constr(self.device) 
            ...
            for run in range(num_runs):
                ...
                if args.trained_on_strategy == 'basic':
                    if SUB_STRATEGY == 'basic':
                        ...
                        # save_path = os.path.join(self.run_test_dir, f'{strategy}_fix_new')
                    else:
                        save_path = os.path.join(self.run_test_dir, strategy, SUB_STRATEGY)
                        # save_path = os.path.join(self.run_test_dir, f'{strategy}_fix_new', SUB_STRATEGY)
                else:
                ...
                dataset = TestFewshotICLDataset(train_path, test_path, label_col, file_col, self.args.dataset)
                sampler = TestCategoriesICLSampler(dataset.support_labels, dataset.query_labels, self.args.eval_iters, n_way, k_shot, query_size)
                ...
                for i, task in enumerate(loader):
                    ...
                    train_x = [train_x[idx] for idx in indices]
                    train_y = [train_y[idx] for idx in indices]

                    task_type = dataset.task_type

                    acc, predictions, golden = model.evaluate(classes, task_type, train_x, train_y, test_x, test_y, batched=self.args.dataset != '20News')

                    self.clprint(f"Iteration: {i+1}\tTest acc: {acc:.4f}\tTest time: {time.time() - ttime} seconds") 
                        
                            
                    with open(test_predictions_file, "a", newline="") as f:
                        writer = csv.writer(f)
                        for j in range(len(golden)):
                            writer.writerow([golden[j], predictions[j]])
                ...
                lb, _ = st.t.interval(confidence=0.95, df=len(test_accuracies)-1, loc=np.mean(test_accuracies), scale=st.sem(test_accuracies)) 
                conf_interval = str(np.mean(test_accuracies) - lb)
                with open(test_scores_file, "a", newline="") as f:
                    writer = csv.writer(f)
