...
from DataLoader.data_loader_within_domain_fsl import CategoriesSampler, \
    FewshotDataset, extract_info, train_test_split_classes, process_labels, extract_train_valid_test_info, extract_strategy_samples_info, extract_cartography_change_samples_info, TestFewshotDataset, TestCategoriesSampler, TestFewshotDatasetSetSupport
...
class WithinDomainFewShotLearningExperiment:
    ...
    def configure(self):
        ...
        if not strategy == 'basic':
            train_path = extract_strategy_samples_info(self.data_dir, self.args.dataset, strategy, 0)
        ### ----------------------------------------------------------------------
    ...
    def run(self):
        ...
        for run in range(self.args.runs):
            ...
            if self.args.validate:
                self.val_results_file = os.path.join(self.run_res_dir, "experiment_results_validation.csv")
    ...
    def forward(self):
        ...
        # model_load_path = os.path.join(self.run_test_dir, 'model.pkl')
        model = self.model_constr(**self.conf)
        with open(model_load_path, 'rb+') as file:
            state = pickle.load(file)
        model.load_state(state)

        with open(os.path.join(self.data_dir, args.dataset, 'basic_scores.pkl'), 'rb') as file:
        # with open(os.path.join(self.data_dir, args.dataset, 'cartography_scores.pkl'), 'rb') as file:
            basic = pickle.load(file)

        train = pd.read_csv(os.path.join(self.data_dir, args.dataset, 'train.csv'))
        strategies = basic.keys()

        cycle = 0
        no_change_in_results = False
        selected_strategy = set()
        score_to_compare = 0

        while not no_change_in_results:
            num_runs = 1

            save_path = os.path.join(self.run_test_dir, 'forward_selection')
            if not os.path.exists(save_path):
                os.makedirs(save_path)

            if cycle > 0 and os.path.exists(os.path.join(save_path, f'{cycle - 1}_best_score.pkl')):
                    with open(os.path.join(save_path, f'{cycle - 1}_best_score.pkl'), 'rb') as file:
                        selected_strategy = set(pickle.load(file)['strategies'])

            count = -1
            for strategy in strategies:
                if strategy in selected_strategy:
                    continue
                count += 1
                file_names = []
                for cls in basic.keys():
                    rating = np.array(basic[cls][strategy])
                    
                    for strat in selected_strategy:
                        rating += np.array(basic[cls][strat])
                    rating = rating / (len(selected_strategy) + 1)

                    for idx in np.argsort(-rating)[:10]:
                        file_names.append(basic[cls]['file_names'][idx])
                sub_train = train[train.FILE_NAME.isin(file_names)]

                
                if os.path.exists(os.path.join(save_path, f'{cycle}_selection_test_scores.csv')):
                    already_run_tests = pd.read_csv(os.path.join(save_path, f'{cycle}_selection_test_scores.csv'))
                    if count < already_run_tests.shape[0]:
                        print(f'Run for strategy {strategy} and model {self.args.model} is already done. Skipping!')
                        continue
                self.set_seed(485443)
                ...
                dataset = TestFewshotDatasetSetSupport(img_path, sub_train, test_path, label_col, file_col)
                ...
                # Store test scores
                test_scores_file = os.path.join(save_path, f'{cycle}_selection_test_scores.csv')
                score_name = "accuracy"
                if count == 0:
                    with open(test_scores_file, "w+", newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow(["strategy", f"mean_{score_name}", f"median_{score_name}", "95ci", "time"])
                ...
                with open(test_scores_file, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([strategy, mean, median, conf_interval, ttime])

                self.clprint(f"\nRun {strategy} done, mean {score_name}: {mean}, median {score_name}: {median}, 95ci: {conf_interval}, time(s): {ttime}")
                self.clprint("-"*40)
            

            if cycle > 0 and os.path.exists(os.path.join(save_path, f'{str(cycle - 1)}_best_score.pkl')):
                with open(os.path.join(save_path, f'{cycle - 1}_best_score.pkl'), 'rb') as file:
                    score_to_compare = pickle.load(file)['score']
            finished_scores = pd.read_csv(os.path.join(save_path, f'{cycle}_selection_test_scores.csv'))

            best_diff, best_score = -1, -1
            best_strategy = None
            for score_idx, row in finished_scores.iterrows():
                diff = row['mean_accuracy'] - score_to_compare
                if diff > best_diff:
                    best_diff = diff
                    best_score = row['mean_accuracy']
                    best_strategy = row['strategy']
            if best_diff > 0:
                selected_strategy.add(best_strategy)
                with open(os.path.join(save_path, f'{cycle}_best_score.pkl'), 'wb') as file:
                    score_to_compare = pickle.dump({'score': best_score, 'strategies': selected_strategy}, file)
                cycle += 1
            else:
    ...
    def backward(self):
        ...
        no_change_in_results = False
        selected_strategy = set(strategies)
        score_to_compare = 0
        cycle = 0

        save_path = os.path.join(self.run_test_dir, 'backward_selection')
        # save_path = os.path.join(self.run_test_dir, 'backward_selection_cart')
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        if not os.path.exists(os.path.join(save_path, f'{cycle - 1}_best_score.pkl')):
            file_names = []
            for cls in basic.keys():
                rating = None
                for strat in selected_strategy:
                    if rating is None:
                        ...
                        rating += np.array(basic[cls][strat])
                rating = rating / (len(selected_strategy))
            ...
            self.clprint(f"\nRun first done, mean; {mean}, median {median}, 95ci: {conf_interval}, time(s): {ttime}")
            self.clprint("-"*40)
            with open(os.path.join(save_path, f'{cycle - 1}_best_score.pkl'), 'wb') as file:
                pickle.dump({'score': mean, 'strategies': selected_strategy}, file)

        while not no_change_in_results:
            num_runs = 1

            if cycle > 0 and os.path.exists(os.path.join(save_path, f'{str(cycle - 1)}_best_score.pkl')):
                with open(os.path.join(save_path, f'{str(cycle - 1)}_best_score.pkl'), 'rb') as file:
                    selected_strategy = set(pickle.load(file)['strategies'])

            print(selected_strategy)
            count = -1
            for strategy in strategies:
                if strategy not in selected_strategy:
                    continue
                count += 1
                file_names = []
                for cls in basic.keys():
            ...
            for score_idx, row in finished_scores.iterrows():
                ...
                if diff > best_diff:
                    ...
                    best_strategy = row['strategy']
            if best_diff > 0:
                selected_strategy.remove(best_strategy)
                with open(os.path.join(save_path, f'{cycle}_best_score.pkl'), 'wb') as file:
                    pickle.dump({'score': best_score, 'strategies': selected_strategy}, file)
                cycle += 1
            else:
                no_change_in_results = True
                break
