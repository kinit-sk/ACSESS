...
class Cal(EarlyTrain):
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200, specific_model=None,
                 balance=True, metric="euclidean", neighbors: int = 10, pretrain_model: str = "TextTransformer", **kwargs):
    ...
    def find_knn(self):
        ...
        model = nets.__dict__[self.pretrain_model](model_name=self.pretrain_model, num_classes=self.args.num_classes, record_embedding=True, no_grad=True).to(self.args.device)
        model.eval()
    ...
    def calc_kl(self, knn, index=None):
        ...
        # print(self.args.selection_batch)
        # print(probs.shape)
        ...
        for i in range(0, sample_num, self.args.selection_batch):
            ...
            aa = aa[:bb.shape[0], :bb.shape[1], :bb.shape[2]]
            # print(aa.shape)
            # print(bb.shape)
    ...
    def finish_run(self):
        ...
        if self.balance:
            ...
            for c, knn in zip(range(self.args.num_classes), self.knn):
                class_index = np.arange(self.n_train)[np.array(self.dst_train.targets) == c]
                budget = self.fraction if self.fraction > 1 else round(self.fraction * len(class_index))
        ...
        return {"indices": selection_result, "scores": [score.tolist() for score in scores]}
