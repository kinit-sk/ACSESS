...
class EarlyTrain(CoresetMethod):
    ...
    def run(self):
        ...
        self.model = nets.__dict__[self.args.core_model if self.specific_model is None else self.specific_model](model_name=self.args.core_model, num_classes=self.dst_pretrain_dict["num_classes"] if self.if_dst_pretrain else self.num_classes).to(self.args.device)
