...
class Herding(EarlyTrain):
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200,
                 specific_model="TextTransformer", balance: bool = False, metric="euclidean", **kwargs):
        ...
        if self.balance:
            ...
            for c in range(self.args.num_classes):
                ...
                selection_result = np.append(selection_result, self.herding(self.construct_matrix(class_index),
                        budget=self.fraction if self.fraction > 1 else round(self.fraction * len(class_index)), index=class_index))
        else:
