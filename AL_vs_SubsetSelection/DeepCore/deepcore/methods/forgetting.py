...
class Forgetting(EarlyTrain):
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200, specific_model=None, balance=True,
        ...
        super().__init__(dst_train, args, fraction, random_seed, epochs, specific_model=specific_model,
                         dst_test=dst_test, torchvision_pretrain=kwargs['torchvision_pretrain'])
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        with torch.no_grad():
            ...
            self.forgetting_events[torch.tensor(batch_inds).cuda()[(self.last_acc[batch_inds]-cur_acc)>0.01]]+=1.
    ...
    def select(self, **kwargs):
        ...
        if not self.balance:
            ...
            for c in range(self.num_classes):
                ...
                if self.fraction > 1:
                    budget = self.fraction
                else:
                    budget = round(self.fraction * len(c_indx))
                top_examples = np.append(top_examples, c_indx[np.argsort(self.forgetting_events[c_indx].cpu().numpy())[::-1][:budget]])

        return {"indices": top_examples, "scores": self.forgetting_events.cpu().numpy()}, warmup_test_acc
