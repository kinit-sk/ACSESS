...
class DeepFool(EarlyTrain):
    ...
    def finish_run(self):
        ...
        if self.balance:
            ...
            for c in range(self.args.num_classes):
                ...
                budget = self.fraction if self.fraction > 1 else round(len(class_index) * self.fraction)
                selection_result = np.append(selection_result, class_index[
                    r[class_index].argsort()[:budget]])
        else:
    ...
    def deep_fool(self, inputs):
        ...
        # print(inputs.shape)
        # print(inputs)
        inputs = self.model.get_only_bert_output(inputs.to(self.args.device))
        ...
        first_preds = self.model(cur_inputs, True).argmax(dim=1)
        self.model.no_grad = False

        for i in range(self.max_iter):
            f_all = self.model(cur_inputs, True)
            ...
            cur_inputs.data += r_i.reshape([r_i.shape[0]] + list(original_shape))

            # Re-input the updated sample into the network and get new predictions.
            self.model.no_grad = True
            preds = self.model(cur_inputs, True).argmax(dim=1)
            self.model.no_grad = False
