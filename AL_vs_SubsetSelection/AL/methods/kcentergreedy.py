    ...
    def run(self):
        if self.balance:
            ...
            for c in range(self.args.num_classes):
                class_index = np.arange(self.n_unlabeled)[np.array(self.dst_unlabeled.targets) == c]
                ...
                selection_result = np.append(selection_result, class_index[np.argsort(scores[-1])[::-1][:to_select]])
        else:
