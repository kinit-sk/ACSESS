        ...
        if self.selection_method == 'Loss':
            if self.balance:
                ...
                for c in range(self.args.num_classes):
                    class_index = np.argwhere(true_y == c).reshape(-1)
                    class_scores = scores[class_index]
                    class_indices = np.argsort(-class_scores).reshape(-1)[:to_select]
                    q_indices = np.append(q_indices, class_index[class_indices])
            else:
            ...
            if self.balance:
                ...
                for c in range(self.args.num_classes):
                    class_index = np.argwhere(true_y == c).reshape(-1)
                    class_true = true_y[class_index]
                    class_pred = pred_y[class_index]
                    class_scores = scores[class_index]
                    wrong_idx = np.where(class_true != class_pred)[0]
                    correct_idx = np.where(class_true == class_pred)[0]
                    selected = class_index[wrong_idx[np.argsort(class_scores[wrong_idx])[:to_select]]]
                    q_indices = np.append(q_indices, selected)
                    if len(selected) < to_select:
                        t_selected = class_index[correct_idx[np.argsort(class_scores[correct_idx])[:(to_select - len(selected))]]]
                        q_indices = np.append(q_indices, t_selected)
            else:
